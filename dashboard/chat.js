...
@media (max-width: 768px) {
  ...
  .chat-window.mobile-open { display: flex; position:fixed; inset:0; z-index:50; }
    ...
    return `
      <div class="conv-item ${isActive ? 'active' : ''}" onclick="openPrivateChat('${other?.id}')">
        ...
        <div class="conv-info">
          ...
          <div class="conv-last">${esc(c.last_message||"Start chatting")}</div>
        </div>
        <div class="conv-meta">
...
window.openPrivateChat = async function(userId) {
  ...
  if (!conv) {
    const { data } = await sb.from("conversations").insert({
      user_a: currentUser.id, user_b: userId, last_message: ""
    }).select().single();
...
async function sendMsg() {
  ...
  if (activeConvId === "__team__") {
    ...
    await sb.from("conversations").update({ last_message: text, last_msg_at: new Date().toISOString() })
      .eq("id", activeConvId);
  }
}
