...
.chat-sidebar-title {
  font-family: -apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif; font-size: 18px; font-weight: 800;
  margin-bottom: 12px;
}
.chat-search {
...
.chat-search input {
  ...
  font-family: -apple-system,BlinkMacSystemFont,'SF Pro Text','Segoe UI',sans-serif; font-size: 13px;
...
.chat-tab.active { color: rgba(255,255,255,0.85); border-bottom-color: rgba(255,255,255,0.85); }
...
.chat-input-wrap:focus-within { border-color: rgba(10,132,255,0.5); }

#msgInput {
  flex: 1; background: none; border: none; color: #f0f0ff;
  font-family: -apple-system,BlinkMacSystemFont,'SF Pro Text','Segoe UI',sans-serif; font-size: 14px;
...
.send-btn, .record-btn {
  ...
  display: flex; align-items: center; justify-content: center;
  transition: all 0.2s; box-shadow: 0 4px 14px rgba(10,132,255,0.4);
}
.send-btn:hover, .record-btn:hover { transform: scale(1.05); box-shadow: 0 6px 20px rgba(10,132,255,0.5); }
...
.team-header-badge {
  ...
  border-radius: 20px; padding: 4px 12px; font-size: 11px; font-weight: 700; color: rgba(255,255,255,0.8);
}
...
@media (max-width: 768px) {
  .chat-layout { margin: 0 !important; height: calc(100dvh - 56px - 60px); min-height: 300px; flex-direction: column; }
  .chat-sidebar { width: 100%; border-right: none; }
  .chat-window { display: none; }
  .chat-sidebar.hidden { display: none; }
  .chat-window.mobile-open { display: flex; position: fixed; inset: 0; z-index: 200; background: #080810; flex-direction: column; }
...
async function initChat(targetUserId = null) {
  ...
  const needsInit = !chatInitialized || !container.innerHTML.trim();
  if (needsInit) {
  ...
  // If called with a user to chat with, open that chat
  if (targetUserId) {
    await window.openPrivateChat(targetUserId);
  }
}
...
async function loadConversations() {
  const { data } = await sb.from("conversations")
    .select(`*, user1_profile:users!conversations_user1_id_fkey(*), user2_profile:users!conversations_user2_id_fkey(*)`)
...
function renderConvList(filter = "") {
  ...
  const filtered = conversations.filter(c => {
    const other = c.user1_id === currentUser.id ? c.user2_profile : c.user1_profile;
...
window.openPrivateChat = async function(userId) {
  ...
  if (!conv) {
    const { data } = await sb.from("conversations").insert({
      user1_id: currentUser.id, user2_id: userId, last_msg: ""
    }).select().single();
    ...
    html += `
      <div class="msg-row ${isMine ? 'mine' : ''}">
        ...
        <div class="bubble ${isMine ? 'mine' : 'theirs'}">
          ${mode === "team" && !isMine ? `<div style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.85);margin-bottom:4px">${esc(sender?.name||"?")}</div>` : ""}
          ${content}
