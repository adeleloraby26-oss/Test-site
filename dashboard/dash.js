...
const IC = {
  verify: `<svg class="verify" viewBox="0 0 24 24" fill="#6366f1" stroke="none" style="width:14px;height:14px;flex-shrink:0"><path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0 1 12 2.944a11.955 11.955 0 0 1-8.618 3.04A12.02 12.02 0 0 0 3 9c0 5.591 3.824 10.29 9 11.622C17.176 19.29 21 14.591 21 9c0-1.042-.133-2.052-.382-3.016z"/></svg>`,
...
async function loadProfile() {
  ...
  const rank  = data.ranks || getRankForPoints(data.points);
...
async function loadMembers() {
  const { data } = await sb.from("users").select("*,ranks(*)").order("points",{ascending:false});
  if (!data) return;
  allMembers = data;
...
function renderMemberLb() {
  ...
  list.innerHTML = allMembers.map((m, i) => {
    const rank = m.ranks || getRankForPoints(m.points||0);
...
async function openMemberModal(userId) {
  ...
  const rank  = member.ranks || getRankForPoints(member.points||0);
