...
function verifyBadge(size) {
  size = size || 16;
  return '<i class="fa-solid fa-circle-check verify-badge" style="font-size:' + size + 'px;" title="Verified Account"></i>';
}
...
(async function() {
  try {
    ...
    if (data && data.session) {
      ...
      if (c) c.innerHTML = '<div class="empty-state"><h3>Session Expired</h3><p>Please sign in to continue.</p><br><a href="../auth/index.html" class="btn-logout" style="background:var(--apple-blue);color:#fff;border:none;padding:10px 24px;text-decoration:none;border-radius:10px;display:inline-block">Sign In</a></div>';
    }
  } catch(e) {
...
window.uploadAvatar = async function(input) {
  ...
  try {
    ...
    await sb.from("users").update({ avatar_url: publicUrl }).eq("id", uid);

    if (avatarCircle) {
      const oldContent = avatarCircle.querySelector("span, img");
    ...
    if (statusEl) { statusEl.textContent = "Upload failed"; statusEl.style.color = "#ff3b30"; }
  }
};
...
window.saveBio = async function() {
  ...
  const { error } = await sb.from("users").update({ bio: bio.trim() }).eq("id", session.user.id);
...
function renderLeaderboard(list) {
  ...
  el.innerHTML = list.map(function(m, i) {
    ...
    const bioLine = m.bio
      ? '<div class="lb-bio">' + esc(m.bio) + '</div>'
      : "";
...
function renderProfilesGrid(list) {
  ...
  el.innerHTML = list.map(function(m, i) {
    ...
    const bioLine = m.bio
      ? '<div class="pcard-bio">' + esc(m.bio) + '</div>'
      : "";
...
// ====================================================
// OPEN PROFILE MODAL  — tasks HIDDEN from others
// ====================================================
window.openProfile = async function(uid) {
  ...
  pmBioEl.style.display = m.bio ? "block" : "none";
  ...
  // Tasks: show COUNT only, no content
