...
.profile-bio {
  font-size: 14px; color: rgba(240,240,255,0.7); line-height: 1.65;
  max-width: 480px;
}
...
.rpc-title { font-family:'Syne',sans-serif; font-size:15px; font-weight:700; }
...
.titles-title { font-family:'Syne',sans-serif; font-size:15px; font-weight:700; margin-bottom:12px; }
...
.settings-hdr {
  padding: 16px 20px; font-family:'Syne',sans-serif; font-size:13px; font-weight:700;
...
.settings-save-btn:hover { transform:translateY(-1px); box-shadow:0 8px 24px rgba(99,102,241,0.45); }
...
.confirm-reason {
  ...
  background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.09);
  border-radius:12px; color:#f0f0ff; font-family:'DM Sans',sans-serif; font-size:13px; outline:none;
...
.btn-cancel-confirm {
  ...
  color:#f0f0ff; font-family:'Syne',sans-serif; font-size:13px; font-weight:700;
    ...
    <div class="profile-page">
      ...
      <div class="profile-info">
        <div class="profile-name">
          ${esc(p.name||"")}
          ${p.verified ? '<i class="fa-solid fa-circle-check" style="color:#6366f1;font-size:18px"></i>' : ""}
        </div>
        ...
        <div class="profile-bio">${esc(p.bio || "No bio yet. Click Settings to add one.")}</div>
      </div>
      ...
      <div class="rank-progress-card">
        <div class="rpc-header">
          <div class="rpc-title">${rank.icon} ${rank.name} → ${nextRank ? nextRank.icon + " " + nextRank.name : "MAX"}</div>
      ...
      setTimeout(() => { const f=document.getElementById('profileProgressFill'); if(f) f.style.width='${pct}%'; }, 100);
...
async function uploadAvatar(input) {
  ...
  await sb.from("users").update({ avatar_url: data.publicUrl + "?t=" + Date.now() }).eq("id", currentUser.id);
  await loadProfile();
  renderProfile();
...
function renderSettings() {
  ...
  container.innerHTML = `
    <div class="settings-page">
      <div style="font-family:'Syne',sans-serif;font-size:22px;font-weight:800;margin-bottom:20px">Settings</div>
